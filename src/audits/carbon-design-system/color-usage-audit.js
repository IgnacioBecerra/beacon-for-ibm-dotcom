 ...
 class ColorUsageAudit extends Audit {
	 ...
	 static audit(artifacts) {
		...
		loadMetrics.stylesheets.forEach( rules => {
			const filteredStyles = rules.content.replace(/\n/g, '').split(';').filter(e => e.trim().startsWith('--cds'));

			filteredStyles.forEach(style => {
				const str = style.split(':');
				str[0] = str[0].trim();
				let tokenValue = !str[1].startsWith('var') ? str[1] : str[1].match(/\,(.*)\)/)[1];
		...
		for(let key in tokenArray) {
			if(colorJson.hasOwnProperty(key)) {
				let difference = tokenArray[key].filter(x => !colorJson[key].includes(x))
